...
# API 
from .text_extractor_by_api import API_INFO_EXTRACTOR
...
def text_to_info_finder(text):
    ...
    # print("Info:: " , text)
    # call API Calling Info Extractor
...
def extract_text_from_file(file_data, file_type):
    try:
        if file_type == 'image':
            return image_to_text_with_gimini(file_data)
        elif file_type == 'pdf':
            return pdf_to_page_extraction(file_data)
